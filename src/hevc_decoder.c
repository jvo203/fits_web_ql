...
//av_parser_init cannot find the HEVC parser! disabling for now
/*
EMSCRIPTEN_KEEPALIVE
static double hevc_decode_nal_stream(const unsigned char *data, size_t data_len, unsigned char *canvas, unsigned int _w, unsigned int _h, const unsigned char *alpha, const char *colourmap)
{
    if (avctx == NULL || avpkt == NULL || avframe == NULL)
        return 0.0;
    ...
    if (!parser)
    {
        printf("[wasm hevc] av_parser not found\n");
        return 0.0;
    }
    ...
    printf("HEVC: parsing a NAL stream of length %zu bytes\n", data_len);
    ...
    while (data_size > 0)
    {
        int ret = av_parser_parse2(parser, avctx, &avpkt->data, &avpkt->size, data_buf, data_size, AV_NOPTS_VALUE, AV_NOPTS_VALUE, 0);

        if (ret < 0)
        {
            printf("[wasm hevc] error while parsing; ret = %d\n", ret);
            return 0.0;
        }
        ...
        printf("HEVC: decoding a NAL unit of length %d bytes\n", avpkt->size);

        ret = avcodec_send_packet(avctx, avpkt);

        printf("[wasm hevc] avcodec_send_packet ret = %d\n", ret);
        ...
        while ((ret = avcodec_receive_frame(avctx, avframe)) == 0)
        {
            ...
            printf("[wasm hevc] decoded a %d x %d frame with in a colourspace:format %d:%d, applying %s colourmap\n", avframe->width, avframe->height, cs, format, colourmap);
