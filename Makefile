...
#--target=avx2-i32x16
simd:
	ispc -O3 --pic --opt=fast-math:balanced --addressing=64 src/fits.ispc -o $(NATIVE)/fits.o -h $(NATIVE)/fits.h
	#rm $(NATIVE)/libfits.a
...
EMFLAGS+=-s MODULARIZE=1 -s 'EXPORT_NAME="FPZIP"' -s 'USE_ZLIB=1'

fpunzip:
	em++ -Oz $(EMFLAGS) $(DEFS) $(SRC) src/main.cc -o build/fpzip.$(FPZIP_STRING).js $(INCLUDE) -L$(LIBRARY) $(CXXFLAGS) $(LDFLAGS)
